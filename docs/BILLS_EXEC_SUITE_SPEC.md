...
**Status:** **Assignments (slice 1)** implemented (`bill_assignments` + **Assigned work** / **Assigned to me**). **Research** tab: **SPAN** corpus (`get_bills_research()` RPC, doc/PDF embeds, by state), **Legislature (LegiScan)** lookup, and **Compare** (v1: two-pane SPAN + LegiScan with separate compare-side LegiScan state). **Outreach** still planned. This doc captures the agreed product direction from the exec brief and follow-up decisions.
