-- School grade on member records (from application at import; backfilled from applications).

ALTER TABLE public.members
  ADD COLUMN IF NOT EXISTS grade TEXT;

COMMENT ON COLUMN public.members.grade IS 'Applicant school grade (e.g. HS Junior). Copied from applications on import; optional for legacy members.';

-- Backfill from most recent application per personal email (matches original_email).
UPDATE public.members m
SET grade = src.grade
FROM (
  SELECT DISTINCT ON (lower(trim(a.email)))
    lower(trim(a.email)) AS email_key,
    NULLIF(trim(a.grade), '') AS grade
  FROM public.applications a
  WHERE a.grade IS NOT NULL AND trim(a.grade) <> ''
  ORDER BY lower(trim(a.email)), a.submitted_at DESC NULLS LAST
) src
WHERE m.grade IS NULL
  AND lower(trim(coalesce(m.original_email, ''))) = src.email_key;
